 ...
 */
public interface ClubLongRepo extends JpaRepository<ClubLongEntity, Long> {

    @Query(value = "select e from ClubLongEntity e where e.clubName = :clubName and e.deleted = false")
    List<ClubLongEntity> findByName(String clubName);

    @Query(value = "select e from ClubLongEntity e where e.clubName = ?1 and e.deleted = false")
    List<ClubLongEntity> findByName2(@Param("clubName") String clubName);
